...
// // // Shape returned by /executive/kpis — iconColor is NOT in the backend response
// // interface ExecutiveKpiDto {
// //   label: string;
// //   shortLabel?: string;
// //   value: string;
// //   change: number;
...
// // interface ExecutiveKpisResponse {
// //   kpis: ExecutiveKpiDto[];
// //   meta?: Record<string, unknown>;
// // }

// // // Internal type used by renderKpiCards — iconColor is optional with a fallback
// // interface KpiCardItem {
// //   label: string;
// //   value: string;
// //   change: number;
// //   icon: string;
// //   iconColor?: string;           // optional — backend doesn't send this
...
// // // Default icon colors per backend icon string
...
// // // Maps dateRange value → human-readable label for KPICard changeLabel
...
// //   const handleExport = () => {
// //     toast({ title: "Exporting dashboard", description: "Your dashboard data is being prepared for download." });
// //   };
...
// //   // Fallback KPIs shown when the API errors
// //   const fallbackKpis: KpiCardItem[] = [
// //     { label: "Image-Description Mismatch", value: "3.2%",   change: -12, icon: "alert",     iconColor: "text-destructive", status: "danger"  },
// //     { label: "AI Photoshoot Savings",       value: "₹18.5L", change:  28, icon: "camera",    iconColor: "text-success",     status: "success" },
// //     { label: "Compliance Score",            value: "94/100", change:   5, icon: "shield",    iconColor: "text-primary",     status: "info"    },
// //     { label: "Localization Complete",       value: "87%",    change:  15, icon: "translate", iconColor: "text-info",        status: "success" },
// //     { label: "SKU AI-Coverage",             value: "92%",    change:   8, icon: "cpu",       iconColor: "text-primary",     status: "success" },
// //     { label: "Revenue at Risk",             value: "₹2.3Cr", change: -22, icon: "warning",   iconColor: "text-warning",     status: "warning" },
// //   ];

// //   const renderKpiCards = (items: KpiCardItem[], fromBackend: boolean) =>
// //     items.map((kpi, index) => {
// //       let statusColor = kpi.status;
// //       if (!statusColor) {
// //         if (kpi.label.includes("Savings") || kpi.label.includes("Coverage") || kpi.label.includes("Complete")) {
// //           statusColor = kpi.change >= 0 ? "success" : "warning";
// //         } else if (kpi.label.includes("Risk") || kpi.label.includes("Mismatch")) {
// //           statusColor = kpi.change >= 0 ? "danger" : "success";
// //         } else if (kpi.label.includes("Compliance")) {
// //           statusColor = "info";
// //         }
// //       }

// //       // FIX: fall back to iconColorMap if iconColor is not provided by backend
// //       const resolvedIconColor = kpi.iconColor ?? iconColorMap[kpi.icon] ?? "text-primary";
...
// //             value={kpi.value}
// //             change={kpi.change}
// //             changeLabel={DATE_LABEL_MAP[dateRange]}
// //             icon={iconMap[kpi.icon] ?? AlertTriangle}
// //             iconColor={resolvedIconColor}
// //             delay={index * 50}
// //             isFromBackend={fromBackend}
// //             statusColor={statusColor}
// //           />
// //         </div>
// //       );
...
// //                 Share
// //               </Button>
// //               <Button variant="outline" size="sm" onClick={handleExport} className="h-10 hover:bg-muted/50 transition-colors">
// //                 <Download className="w-4 h-4 mr-2" />
// //                 Export
// //               </Button>
// //             </div>
...
// //               >
// //                 <div className="flex items-start gap-4">
// //                   <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110", insight.iconBg)}>
// //                     <Icon className={cn("w-6 h-6", insight.iconColor)} />
// //                   </div>
// //                   <div className="flex-1 min-w-0">
// //                     <p className="text-sm font-semibold text-foreground leading-relaxed">{insight.message}</p>
// //                     <div className="flex items-center gap-1 mt-2">
// //                       <ChevronRight className={cn("w-4 h-4 transition-transform duration-300 group-hover:translate-x-1", insight.iconColor)} />
// //                       <span className={cn("text-xs font-medium", insight.iconColor)}>Learn more</span>
// //                     </div>
...
// //             {isLoading && !kpis.length && (
// //               Array.from({ length: 6 }).map((_, index) => (
// //                 <div key={index} className="h-48 rounded-xl bg-muted/50 animate-pulse border border-border/50" />
// //               ))
// //             )}
// //             {!isLoading && !isError && renderKpiCards(
// //               kpis.map((k) => ({
// //                 label:  k.shortLabel ?? k.label,
// //                 value:  k.value,
// //                 change: k.change,
// //                 icon:   k.icon,
// //                 // iconColor intentionally omitted — resolved inside renderKpiCards via iconColorMap
// //                 status: k.status as KpiCardItem["status"],
// //               })),
// //               true,
// //             )}
// //             {isError && renderKpiCards(fallbackKpis, false)}
// //           </div>
// //         </div>
...
// //         <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
// //           <div className="animate-fade-in" style={{ animationDelay: "300ms", animationFillMode: "forwards", opacity: 0 }}>
// //             <QualityRiskRadar dateRange={dateRange} />
// //           </div>
// //           <div className="animate-fade-in" style={{ animationDelay: "400ms", animationFillMode: "forwards", opacity: 0 }}>
// //             <PhotoshootPerformance dateRange={dateRange} />
// //           </div>
// //         </div>
// //       </div>
// //     </div>
...
//       // ── Meta section ──────────────────────────────────────────────
//       rows.push(["AI E-Commerce Content Intelligence – Dashboard Export"]);
//       rows.push(["Generated at", new Date().toLocaleString()]);
//       rows.push(["Date Range", DATE_LABEL_MAP[dateRange] ?? dateRange]);
//       rows.push([]);

//       // ── Quick Insights ────────────────────────────────────────────
//       rows.push(["QUICK INSIGHTS"]);
//       rows.push(["Type", "Message"]);
//       quickInsights.forEach((insight) => {
//         rows.push([insight.type.toUpperCase(), insight.message]);
//       });
//       rows.push([]);

//       // ── KPI Cards ─────────────────────────────────────────────────
//       rows.push(["KEY PERFORMANCE INDICATORS"]);
//       rows.push(["Metric", "Value", "Change (%)", "Status", "Source"]);

//       const activeKpis: KpiCardItem[] =
//         !isLoading && !isError && kpis.length
//           ? kpis.map((k) => ({
//               label: k.shortLabel ?? k.label,
//               value: k.value,
//               change: k.change,
...
//           kpi.status ?? "",
//           !isLoading && !isError && kpis.length ? "Live" : "Demo",
//         ]);
//       });
...
//       link.href = url;
//       link.download = `content-intelligence-${dateRange}-${new Date().toISOString().slice(0, 10)}.csv`;
...
//         description: "Dashboard data has been downloaded as a CSV file.",
//       });
//     } catch (err) {
...
import { EXECUTIVE_API_URL } from "@/lib/api-config";
import { useFilters } from "@/contexts/FilterContext";
import { exportDashboardPDF } from "@/pages/ExportdashboardPdf";
  ...
  const handleExport = () => {
    try {
      ...
      exportDashboardPDF({
        dateRange,
        kpis: activeKpis,
        quickInsights: quickInsights.map((ins) => ({
          type:    ins.type,
          message: ins.message,
        })),
      });
      ...
      toast({
        ...
        description: "Use your browser'Save as PDF' option in the print dialog.",
      });
    } catch (err) {
      console.error("Export failed:", err);
      toast({
