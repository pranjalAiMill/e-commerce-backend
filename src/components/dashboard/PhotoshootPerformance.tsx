...
// Shape returned by /executive/photoshoot-performance
interface PhotoshootRaw {
  monthlySavings: { month: string; savingsINR: number }[];
  rejectionRate: {
    avgRate: number;
    byMarketplace: { marketplace: string; rejectionRate: number; skuCount: number }[];
  };
  styleDistribution: { style: string; count: number; growth: number }[];
}

interface PhotoshootPerformanceProps {
  dateRange?: string;
}
...
export function PhotoshootPerformance({ dateRange = "30d" }: PhotoshootPerformanceProps) {
  const { data: raw, isLoading, isError } = useQuery<PhotoshootRaw>({
    queryKey: ["executive", "photoshoot-performance", dateRange],
    queryFn: async () => {
      const params = new URLSearchParams();
      // FIX: correct backend route is /executive/photoshoot-performance
      // FIX: backend Query param is "period", not "date_range"
      params.set("period", dateRange);
      const url = `${EXEC_API_BASE}/executive/photoshoot-performance?${params.toString()}`;
  ...
  // Transform raw API shape → component shape
  const costData: CostDataPoint[] = raw?.monthlySavings.map((m) => ({
    month: m.month,
    saved: m.savingsINR,
  })) ?? [
    { month: "Jan", saved: 125000 },
  ...
  const rejectionData: RejectionDataPoint[] = raw?.rejectionRate.byMarketplace.map((mp, i) => ({
    name: mp.marketplace,
    value: mp.rejectionRate,
  ...
  const styleCards: StyleCard[] = raw?.styleDistribution.map((s) => ({
    name: s.style,
    count: s.count,
    trend: `+${s.growth}%`,
  })) ?? [
  ...
  const isUsingDummy = isError || (!raw && !isLoading);
  const avgRejection = raw?.rejectionRate.avgRate
    ?? Math.round(rejectionData.reduce((s, i) => s + i.value, 0) / rejectionData.length);
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={costData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  cursor={{ fill: "hsl(var(--primary))", fillOpacity: 0.1 }}
                />
                <Bar dataKey="saved" fill="url(#barGradient)" radius={[8, 8, 0, 0]} animationDuration={800} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
