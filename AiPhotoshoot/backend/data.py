...
AZURE_FOLDER    = "AI_PhotoShoot"

BLOB_BASE_URL = f"https://{AZURE_ACCOUNT}.blob.core.windows.net/{AZURE_CONTAINER}/{AZURE_FOLDER}"


def get_smart_image_url(image_id: str) -> str:
    """
    Tries .jpg first, then .jpeg, then falls back to .jpg.
    Same logic as the original local file check — but now checks
    against Azure Blob Storage URLs instead of local filesystem.
    """
    jpg_url  = f"{BLOB_BASE_URL}/{image_id}.jpg"
    jpeg_url = f"{BLOB_BASE_URL}/{image_id}.jpeg"

    # Check if .jpg exists on blob
    try:
        req = urllib.request.Request(jpg_url, method="HEAD")
    ...
    # Check if .jpeg exists on blob
    try:
        req = urllib.request.Request(jpeg_url, method="HEAD")
        with urllib.request.urlopen(req) as resp:
            if resp.status == 200:
                return jpeg_url
    except Exception:
    ...
    # Fallback — return .jpg even if missing (frontend shows broken image)
    return jpg_url
...
def get_template_database():
    """
    Returns the structured dictionary of all photoshoot templates.
    Images are served from Azure Blob Storage.
    Smart URL check: tries .jpg → .jpeg → fallback .jpg
    """
    return {
        "Indian": [
            {
                ...
                "img": get_smart_image_url("in_saree"),
                "prompt": (
                    "Full body fashion shot of an Indian female model wearing the input saree. "
                    "Traditional drape, hand on hip, warm studio lighting, elegant pose."
                )
            },
            {
                "id": "in_kurta",
                ...
                "img": get_smart_image_url("in_kurta"),
                "prompt": (
                    "E-commerce photography of an Indian model wearing the input kurta. "
                    "Standing straight, neutral expression, clean grey background, sharp focus."
                )
            },
            {
                "id": "in_fest",
                ...
                "img": get_smart_image_url("in_fest"),
                "prompt": (
                    "Festive Diwali photoshoot. Indian model wearing the input cloth. "
                    "Background with blurred marigold flowers and warm diya lights. Happy celebration vibe."
                )
            },
            {
                "id": "in_fusion",
                ...
                "img": get_smart_image_url("in_fusion"),
                "prompt": (
                    "Trendy Indo-western fashion shot. Young Indian model wearing the input cloth "
                    "in a modern Mumbai cafe setting. Natural daylight, chic pose."
                )
            }
        ],
        ...
        "South African": [
            {
                ...
                "img": get_smart_image_url("sa_ubuntu"),
                "prompt": (
                    "Outdoor lifestyle shot. South African model with natural hair wearing the input cloth. "
                    "Sunny day, blue sky, urban nature background."
                )
            },
            {
                "id": "sa_safari",
                ...
                "img": get_smart_image_url("sa_safari"),
                "prompt": (
                    "Golden hour photography. South African model wearing the input cloth. "
                    "Background implying a nature reserve or lodge. Warm earth tones."
                )
            },
            {
                "id": "sa_modern",
                ...
                "img": get_smart_image_url("sa_modern"),
                "prompt": (
                    "High fashion studio portrait. Black model wearing the input cloth. "
                    "Bold colorful geometric background. Editorial lighting."
                )
            },
            {
                "id": "sa_cape",
                ...
                "img": get_smart_image_url("sa_cape"),
                "prompt": (
                    "Street style photography in Cape Town. Model leaning against a textured colorful wall. "
                    "Wearing the input cloth. Cool, modern aesthetic."
                )
            }
        ],
        ...
        "Global": [
            {
                ...
                "img": get_smart_image_url("gl_studio"),
                "prompt": (
                    "Standard Amazon e-commerce shot. Professional model wearing the input cloth. "
                    "Pure white background (RGB 255,255,255). Front facing, neutral pose."
                )
            },
            {
                "id": "gl_urban",
                ...
                "img": get_smart_image_url("gl_urban"),
                "prompt": (
                    "Urban streetwear style. Model walking on a blurred city street (New York/London). "
                    "Wearing the input cloth. Overcast soft lighting."
                )
            },
            {
                "id": "gl_min",
                ...
                "img": get_smart_image_url("gl_min"),
                "prompt": (
                    "Minimalist aesthetic. Model wearing the input cloth against a soft pastel beige wall. "
                    "Soft shadows, artistic composition."
                )
            },
            {
                "id": "gl_high",
                ...
                "img": get_smart_image_url("gl_high"),
                "prompt": (
                    "Vogue magazine style. Dramatic side lighting. Model wearing the input cloth "
                    "with an angular, confident pose. Moody dark background."
                )
            }
        ]
    }