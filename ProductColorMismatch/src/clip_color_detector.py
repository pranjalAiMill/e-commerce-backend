...
class ClipColorDetector:
    """
    Detects dominant color of an image using OpenAI GPT Vision.
    """
    ...
    def __init__(
        ...
        self.llm = ChatOpenAI(
            ...
            api_key=openai_api_key,
            temperature=0,
            max_tokens=300  # Increased slightly for richer top_k output
        )
    ...
    def detect_color(
        ...
        """
        Detect color using GPT Vision. Confidence and top candidates
        are generated by the LLM, not hardcoded.
        """
        if candidate_colors is None:
            candidate_colors = COLOR_CANDIDATES
        ...
        prompt = f"""
You are a product color classifier. Look carefully at the dominant color of the product in the image.
 
From this candidate list, pick the top {top_k} closest colors, ordered most to least likely:
{candidate_colors}
 
For EACH candidate assign a confidence score (0.0 to 1.0) that reflects how visually certain you are:
- Use HIGH confidence (0.85–1.0) when the color is obvious and unambiguous in the image.
- Use MEDIUM confidence (0.50–0.84) when the color is likely but the image has ambiguity (shadows, lighting, etc.).
- Use LOW confidence (0.10–0.49) when it is a possible but uncertain match.
- The top candidate's confidence should reflect your actual certainty — do NOT default to a fixed value like 0.7.
- All {top_k} confidence scores must sum to 1.0.
 
Respond ONLY with valid JSON, no markdown fences.
 
{{
    "detected_color": "<best match from candidate list>",
    "confidence": <confidence of the best match, float 0.0–1.0>,
    "top_candidates": [
        {{"color": "<color_name>", "confidence": <float>}},
        {{"color": "<color_name>", "confidence": <float>}},
        {{"color": "<color_name>", "confidence": <float>}}
    ],
    "reason": "<one sentence describing what you see in the image>"
}}
"""
        ...
        try:
            ...
            detected_color = parsed.get("detected_color")
            detected_confidence = float(parsed.get("confidence", 0.0))
            reason = parsed.get("reason", "")
 
            # Parse top_candidates from LLM output
            raw_candidates = parsed.get("top_candidates", [])
            top_candidates: List[Tuple[str, float]] = []
            for entry in raw_candidates:
                color = entry.get("color", "")
                conf = float(entry.get("confidence", 0.0))
                top_candidates.append((color, conf))
 
            # Validate detected_color is in the candidate list
            if detected_color not in candidate_colors:
                for c in candidate_colors:
                    if str(detected_color).lower() == c.lower():
                        detected_color = c
